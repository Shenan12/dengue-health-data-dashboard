...
# --- DATA FOR POINT MAP (Original with Kalmune) ---
...
# Download and fiercely standardize the shapefile names
sl_sf <- read_sf(api_data$simplifiedGeometryGeoJSON) |>
  rename_with(tolower) |>
  mutate(
...
map_data_clean <- annual_district |>
  mutate(
    district_clean = as.character(district),
    district_clean = if_else(district_clean == "Kalmune", "Ampara", district_clean),
    join_name = str_to_lower(str_replace_all(district_clean, "[^a-zA-Z]", "")),
    join_name = case_when(
      join_name == "mullaittivu" ~ "mullaitivu",
      join_name == "hambanthota" ~ "hambantota",
      join_name == "moneragala"  ~ "monaragala",
...
# FIXED: Joined by BOTH join_name and year so it doesn't break the columns
map_data_full <- grid |>
...
```{r}
#| title: "Annual Dengue Cases by District (Filled)"
#| message: false
#| warning: false
...
### Map by Year (with kalmunei as well)

```{r}
#| title: "Annual Dengue Cases by Health District"
#| message: false
#| warning: false

max_cases_pt  <- max(yearly_map_data$annual_cases, na.rm = TRUE)
pal_pt        <- colorNumeric(palette = "Reds", domain = c(0, max_cases_pt))
years_list_pt <- sort(unique(yearly_map_data$year))

m_pt <- leaflet() |>
  addProviderTiles("CartoDB.Positron") |>
  setView(lng = 80.7, lat = 7.8, zoom = 7)

for (yr in years_list_pt) {
  d_pt <- yearly_map_data |> filter(year == yr)
  m_pt <- m_pt |>
    addCircleMarkers(
      data        = d_pt,
      lng         = ~lon,
      ...
      color       = ~pal_pt(annual_cases),
      fillColor   = ~pal_pt(annual_cases),
      fillOpacity = 0.85,
      opacity     = 1,
      popup       = ~paste0(
...
m_pt |>
  addLayersControl(
    baseGroups = as.character(years_list_pt),
    options    = layersControlOptions(collapsed = FALSE)
  ) |>
  addLegend(
    pal      = pal_pt,
    values   = c(0, max_cases_pt),
    title    = "Annual Cases",
    position = "bottomleft"
  )
```
